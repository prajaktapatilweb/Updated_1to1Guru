...
function TeacherFormFields() {
    ...
    const validationSchema = Yup.object().shape({
        ...
        //mobileno: Yup.string().required('Required').matches(/^[0-9\d]+$/, "Only Numbers are allowed for this field ").length(10,"The mobile number of 10 digit is allowed"),
        city: Yup.string().required('Required'),
        //pincode: Yup.string().required('Required').matches(/^[0-9\d]+$/, "Only Numbers are allowed for this field ").length(6,"Pin code is of 6 digit number only"),
    ...
    return (
        <div>
            ...
            <Modal isOpen={modal} toggle={toggle} className="modal-dialog-centered modal-info"
                ...
                <ModalBody>
                    Thank you for your submission {teachername}
                </ModalBody>
                <ModalFooter>
